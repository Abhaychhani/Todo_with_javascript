class Todo {
  constructor(root) {
    ...
    this.backTodoBtn = document.querySelector(".back-todos-btn");
    this.renderTodo();
  }
  getSavedTodo() {
      ...
      this.allTask.forEach((task) => {
        ...
        <li class="task-list-item">
          <h3 class="item-title">
            ...
            ${
              status
                ? "<span class='status-text active'>Completed<span>"
  ...
  renderTodoDetails(index) {
    const { title, content, date, status } = this.allTask[index];

    this.todoDetails.innerHTML = `
    <div class="back-btn-div">
    <button class="back-todos-btn">Back</button>
    ${
      status
        ? "<span class='status-text active'>Completed<span>"
        : "<span class='status-text'>Pending<span>"
